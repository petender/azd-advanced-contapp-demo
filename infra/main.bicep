...
module containerAppsEnv './modules/container-apps-env.bicep' = {
  ...
  params: {
    ...
    // Event Hub with managed identity (SAS disabled)
    useEventHubManagedIdentity: true
    eventHubNamespaceHost: eventHub.outputs.namespaceHost
    eventHubName: 'telemetry'
    eventHubConsumerGroup: 'ingestion'
    storageAccountName: storageAccount.outputs.name
    storageContainerName: 'eventhub-checkpoints'
    // Cosmos DB with managed identity (local auth disabled)
    useCosmosDbManagedIdentity: true
    cosmosDbUrl: cosmosDb.outputs.endpoint
    cosmosDbDatabase: 'telemetry'
    cosmosDbContainer: 'state'
  }
}
...
// ============================================================================
// Storage Account (for Event Hub checkpointing with Dapr)
// ============================================================================
...
// ============================================================================
// Cosmos DB (store processed telemetry data)
// ============================================================================
...
// Ingestion Service - Consumes events from Event Hub
module ingestionService './modules/container-app.bicep' = {
  ...
  params: {
    ...
    targetPort: 8080
    minReplicas: 1  // Keep at least 1 replica for continuous event consumption
    ...
    env: [
      {
        ...
        value: eventHub.outputs.namespaceHost
      }
      {
        name: 'EVENTHUB_NAME'
        value: 'telemetry'
    ...
    secrets: [
      {
        ...
        keyVaultUrl: keyVault.outputs.cosmosDbSecretUri
